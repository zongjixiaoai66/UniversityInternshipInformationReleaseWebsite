        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shixijilu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/gaoxiaoshixixinxi/upload/shixijiluMuBan.xls"
                        >批量导入实习记录数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shixijilu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="shixijiluUploadSuccess"
                                :on-error="shixijiluUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shixijilu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入实习记录数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shixijilu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shixijilu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixijiluUuidNumber"
                                   header-align="center"
                                   label="实习记录编号">
                        <template slot-scope="scope">
                            {{scope.row.shixijiluUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="danweiLianxiren"
                                   header-align="center"
                                   label="联系人">
                        <template slot-scope="scope">
                            {{scope.row.danweiLianxiren}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="danweiLianxiPhone"
                                   header-align="center"
                                   label="联系方式">
                        <template slot-scope="scope">
                            {{scope.row.danweiLianxiPhone}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="实习薪资">
                        <template slot-scope="scope">
                            {{scope.row.shixijiluXinzi}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixijiluKaishiTime"
                                   header-align="center"
                                   label="实习开始时间">
                        <template slot-scope="scope">
                            {{scope.row.shixijiluKaishiTime}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixijiluJieshuTime"
                                   header-align="center"
                                   label="实习结束时间">
                        <template slot-scope="scope">
                            {{scope.row.shixijiluJieshuTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('shixijilu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shixijilu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('shixijilu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                yonghuId : null,
                shixijiluUuidNumber : null,
                shixijiluName : null,
                shixizhiweiTypes : null,
                danweiAddress : null,
                danweiLianxiren : null,
                danweiLianxiPhone : null,
                shixijiluXinzi : null,
                shixijiluKaishiTime : null,
                shixijiluJieshuTime : null,
                shixijiluContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '邮箱': 'yonghuEmail',
                //本表字段
                     '实习记录编号': "shixijiluUuidNumber",
                     '实习单位名称': "shixijiluName",
                     '实习职位': "shixizhiweiValue",
                     '单位位置': "danweiAddress",
                     '联系人': "danweiLianxiren",
                     '联系方式': "danweiLianxiPhone",
                     '实习薪资': "shixijiluXinzi",
                     '实习开始时间': "shixijiluKaishiTime",
                     '实习结束时间': "shixijiluJieshuTime",
                     '上传时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shixijilu',//当前表表名,
                        sumColum : 'shixijilu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shixijilu_name',//分组字符串字段
                        // types : 'shixijilu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['shixijiluDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            shixijiluUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shixijilu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "shixijilu/update",
                        method: "post",
                        data: {
                            id:id,
//                            shixijiluTypes:shixijiluTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
