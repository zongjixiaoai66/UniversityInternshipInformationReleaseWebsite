...
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 企业
            //级联表
            QiyeEntity qiye = qiyeService.selectById(forum.getQiyeId());
            if(qiye != null){
            BeanUtils.copyProperties( qiye , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"
, "qiyeId"
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setQiyeId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
...
//                            forumEntity.setQiyeId(Integer.valueOf(data.get(0)));   //企业 要改的
